...
import gpr.com.gprapplication.service.CreateReferralService;
...
public class CreateReferralServiceTask extends
...
//	private CreateReferralCallback callback;

	public CreateReferralServiceTask(Activity activity, Referral currentReferral) {
		super();
...
//		superActivityToast.show();
	...
	protected List<CreateResponse> doInBackground(String... param) {
		...
		Log.i(TAG, "Attempting to send referral");
//		superActivityToast.setProgress(40);
		publishProgress(param);
		List<CreateResponse> ret;
		try {
			CreateReferralService service = new CreateReferralService(CredentialManager.getInstance().get(null));
			ret = service.getParseJson(Long.toString(currentReferral.getReferringTo()
							.getId()), currentReferral.getPatientName(),
					currentReferral.getPatientContactPhone(),
					currentReferral.getPatientContactEmail(), currentReferral
							.getPatientContactAddress(), currentReferral
							.getReason(), currentReferral.getReferringTo().getFullName());
